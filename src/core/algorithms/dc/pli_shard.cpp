...
static size_t AddValueToHash(TypedColumnData const& column, size_t row) {
    return IndexProvider<T>::GetInstance()->GetIndex(
            column.GetType().GetValue<T>(column.GetData()[row]));
}
...
static std::vector<size_t> ColumnToHash(TypedColumnData const& column) {
    ...
    switch (column.GetTypeId()) {
        case TypeId::kInt:
            ColumnToHashTyped<int>(hashed_column, column);
            break;
        case TypeId::kDouble:
...
void AddTableToHashAndSortProviders(std::vector<TypedColumnData> const& input) {
    for (size_t col = 0; col < input.size(); col++) {
        ...
        switch (column.GetTypeId()) {
            case TypeId::kInt:
                for (size_t row = 0; row < input[0].GetNumRows(); row++)
                    AddValueToHash<int>(column, row);
                break;
            case TypeId::kDouble:
                for (size_t row = 0; row < input[0].GetNumRows(); row++)
                    AddValueToHash<double>(column, row);
                break;
            case TypeId::kString:
                for (size_t row = 0; row < input[0].GetNumRows(); row++)
                    AddValueToHash<std::string>(column, row);
                break;
            default:
                continue;
        }
...
void PliShardBuilder::BuildPliShards(std::vector<TypedColumnData> const& input) {
    ...
    pli_shards_.reserve(cols_num);

    AddTableToHashAndSortProviders(input);

    // TODO: some columns will be empty, cause there could be like a column representing date,
    // with which FastADC doesn't work. Need to think what to do then.
    // Probably nothing, just need to keep that in mind
