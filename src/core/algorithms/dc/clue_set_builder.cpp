...
CommonClueSetBuilder::CommonClueSetBuilder(PredicateBuilder const& pbuilder)
    : predicate_builder_(pbuilder) {
}

ClueSet CommonClueSetBuilder::BuildClueSet(std::vector<PliShard> const& pliShards) {
    ClueSet clue_set;
    ...
    for (size_t i = 0; i < pliShards.size(); i++) {
        for (size_t j = i; j < pliShards.size(); j++) {
            ...
            if (i == j) {
                builder = std::make_unique<SingleClueSetBuilder>(predicate_builder_, pliShards[i]);
            } else {
                builder = std::make_unique<CrossClueSetBuilder>(predicate_builder_, pliShards[i],
                                                                pliShards[j]);
            }
