...
void PredicateBuilder::ProcessColumnPair(size_t i, size_t j,
    ...
    if (joinable || comparable) {
        ...
        LOG(DEBUG) << "[Predicate Space building]: Current column pair: {"
                   << input[i].GetColumn()->ToString() << "," << input[j].GetColumn()->ToString()
                   << "}. It is " << (!joinable ? "not " : "") << "joinable and "
                   << (!comparable ? "not " : "") << "comparable" << std::endl;
    } else {
        LOG(DEBUG) << "[Predicate Space building]: Skipping column pair {"
                   << input[i].GetColumn()->ToString() << "," << input[j].GetColumn()->ToString()
                   << "}";
    }
}
