...
void AssertClusterValues(model::TypedColumnData const& column, std::vector<size_t> const& cluster) {
    ...
    auto get_value = [](mo::TypedColumnData const& column, size_t row) {
        return column.GetType().GetValue<T>(column.GetData()[row]);
    };

    auto expected = get_value(column, cluster.front());
    for (auto row_index : cluster) {
        auto actual = get_value(column, row_index);
        ...
        for (size_t i = 0; i < shard.plis.size(); ++i) {
            ...
            for (auto key : keys) {
                ...
                switch (column.GetTypeId()) {
                    case mo::TypeId::kInt:
                        AssertClusterValues<int>(column, cluster);
                        break;
